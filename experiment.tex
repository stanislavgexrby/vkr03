...
На рисунках~\ref{fig:diagram_simple}--\ref{fig:diagram_complex} представлены примеры визуализации синтаксических диаграмм для различных конструкций RBNF.

\begin{figure}[h]
\centering
% TODO: Добавить скриншот простой диаграммы (последовательность)
\fbox{\parbox{0.7\textwidth}{\centering [МЕСТО ДЛЯ СКРИНШОТА: Диаграмма последовательности] \\ \texttt{stmt : 'begin' , stmtlist , 'end'.} \\ \vspace{2cm}}}
...
\begin{figure}[h]
\centering
% TODO: Добавить скриншот диаграммы с альтернативами
\fbox{\parbox{0.7\textwidth}{\centering [МЕСТО ДЛЯ СКРИНШОТА: Диаграмма альтернативы] \\ \texttt{type : 'int' ; 'float' ; 'string'.} \\ \vspace{2cm}}}
...
\begin{figure}[h]
\centering
% TODO: Добавить скриншот диаграммы с итерацией
\fbox{\parbox{0.7\textwidth}{\centering [МЕСТО ДЛЯ СКРИНШОТА: Диаграмма итерации] \\ \texttt{list : element , @*(',' , element).} \\ \vspace{2cm}}}
...
\begin{figure}[H]
\centering
% TODO: Добавить скриншот сложной диаграммы
\fbox{\parbox{0.7\textwidth}{\centering [МЕСТО ДЛЯ СКРИНШОТА: Сложная диаграмма] \\ \texttt{expr : term , @*(( '+' ; '-' ) , term).} \\ \vspace{2.5cm}}}
\caption{Визуализация сложной конструкции с вложенными операциями}
\label{fig:diagram_complex}
\end{figure}
...
Такая организация обеспечивает чёткое разделение компонентов, упрощает навигацию по коду и позволяет собирать отдельные части проекта независимо (например, только библиотеку без GUI).